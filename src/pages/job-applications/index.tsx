  ...
  const fetcher = useCallback(
    async () =>
      getJobApplications({
        ...
        order: params.order,
        searchTermKeys: ['application_status.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<JobApplicationInterface>>(
    () => `/job-applications?params=${JSON.stringify(params)}`,
    fetcher,
  );
