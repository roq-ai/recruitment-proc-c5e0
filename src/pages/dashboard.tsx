...
import { getCandidateExperiences } from 'apiSdk/candidate-experiences';
import { CandidateExperienceInterface } from 'interfaces/candidate-experience';
import { getInterviewFeedbacks } from 'apiSdk/interview-feedbacks';
import { InterviewFeedbackInterface } from 'interfaces/interview-feedback';
import { getCandidateQualifications } from 'apiSdk/candidate-qualifications';
...
function DashboardPage() {
  ...
  const candidateExperienceStat = useSWR<PaginatedInterface<CandidateExperienceInterface>>(
    () => `/candidate-experiences`,
    () => getCandidateExperiences({ limit: 1, offset: 0 }),
  );

  const interviewFeedbackStat = useSWR<PaginatedInterface<InterviewFeedbackInterface>>(
    () => `/interview-feedbacks`,
    () => getInterviewFeedbacks({ limit: 1, offset: 0 }),
  );

  const candidateQualificationsStat = useSWR<PaginatedInterface<CandidateQualificationsInterface>>(
    () => `/candidate-qualifications`,
    () => getCandidateQualifications({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Candidate Experiences',
      result: candidateExperienceStat?.data?.totalCount,
      isLoading: candidateExperienceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Interview Feedbacks',
      result: interviewFeedbackStat?.data?.totalCount,
      isLoading: interviewFeedbackStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Candidate Qualifications',
      result: candidateQualificationsStat?.data?.totalCount,
      isLoading: candidateQualificationsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
