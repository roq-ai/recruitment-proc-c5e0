...
type ColumnType = ColumnDef<JobOpeningInterface, unknown>;

interface JobOpeningListPageProps {
...
export function JobOpeningListPage(props: JobOpeningListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<JobOpeningInterface>>(
    () => `/job-openings?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: JobOpeningInterface) => {
    if (hasAccess('job_opening', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/job-openings/view/${row.id}`);
    }
  };
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="job_opening" />
        </Flex>

        {hasAccess('job_opening', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/job-openings/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
