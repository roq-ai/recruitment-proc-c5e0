...
import { getSkills, deleteSkillsById } from 'apiSdk/skills';
import { SkillsInterface } from 'interfaces/skills';

type ColumnType = ColumnDef<SkillsInterface, unknown>;
...
export function SkillsListPage(props: SkillsListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<SkillsInterface>>(
  ...
  const handleView = (row: SkillsInterface) => {
    if (hasAccess('skills', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              ...
              {hasAccess('skills', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
              ...
              {hasAccess('skills', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="skills" />
        </Flex>

        {hasAccess('skills', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
